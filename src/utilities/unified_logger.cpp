...
void UnifiedLogger::initialize(const ExaOptions& options) {
    ...
    if (mpi_rank_ == 0) {
        ...
        std::cout << "Start Time: " << std::put_time(std::localtime(&time_t), 
                                                     "%Y-%m-%d %H:%M:%S") 
                 << std::endl;
        std::cout << "==========================================\n" << std::endl;
    }
...
void UnifiedLogger::readerThreadFunc(CaptureContext* ctx) {
    ...
    while (!ctx->stop_reading.load()) {
        ...
        int ret = select(ctx->capture_pipe.read_end().get() + 1, 
                        &read_fds, nullptr, nullptr, &timeout);
        
        if (ret > 0 && FD_ISSET(ctx->capture_pipe.read_end().get(), &read_fds)) {
            // Data is available - read as much as possible
            while (true) {
                ssize_t bytes_read = ::read(ctx->capture_pipe.read_end().get(), 
    ...
    if (mpi_rank_ == 0) {
        ...
        try {
            ...
            for (const auto& entry : std::filesystem::directory_iterator(log_directory_)) {
                ...
                else if (filename.find("mfem") == 0 && filename.find(".log") != std::string::npos) {
                    actual_file_count++;
            ...
            if (actual_file_count > 0) {
                ...
                for (const auto& [type, files] : files_by_type) {
                    std::cout << "\n  " << type << " outputs (" << files.size() << " files):" << std::endl;
