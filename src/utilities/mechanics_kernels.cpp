                ...
                const mfem::Array<int>* local2global)
{
    ...
    RAJA::Layout<DIM4> layout_jacob_input = RAJA::make_permuted_layout({{ dim, dim, nqpts, input_nelems } }, perm4);
    RAJA::View<const double, RAJA::Layout<DIM4, RAJA::Index_type, 0> > J_input(jacobian_data, layout_jacob_input);

    RAJA::Layout<DIM3> layout_field_input = RAJA::make_permuted_layout({{ nnodes, dim, input_nelems } }, perm3);
    RAJA::View<const double, RAJA::Layout<DIM3, RAJA::Index_type, 0> > field_input(field_data, layout_field_input);
    ...
    RAJA::Layout<DIM4> layout_grad_output = RAJA::make_permuted_layout({{ dim, dim, nqpts, nelems } }, perm4);
    RAJA::View<double, RAJA::Layout<DIM4, RAJA::Index_type, 0> > field_grad_view(field_grad_array, layout_grad_output);
            ...
            for (int t = 0; t < dim; t++) {
                for (int s = 0; s < dim; s++) {
                    for (int r = 0; r < nnodes; r++) {
                        for (int q = 0; q < dim; q++) {
                            field_grad_view(q, t, j_qpts, i_local_elem) += 
                                field_input(r, q, i_global_elem) *
