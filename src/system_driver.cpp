...
{
   // Tell the mech_operator to make use of the old Essential True Dofs
   mech_operator->UseEssTDofsOld();
   Vector deltaF(x); deltaF.UseDevice(true);
   {
      ...
      // K_uc * (x - x_prev)_c = deltaF_u
      {
         ...
         auto X = x.Read();
         MFEM_FORALL(i, size, X[I[i]] = X[I[i]] - XPREV[I[i]]; );
      }
      mech_operator->ApplyLocalGradient(xprev, deltaF, b);
      deltaF = b;
      auto I = mech_operator->GetEssentialTrueDofs().Read();
      auto size = mech_operator->GetEssentialTrueDofs().Size();
      auto Y = b.Write();
      auto X = deltaF.Read();
      // Need to get rid of all the unconstrained values here
      MFEM_FORALL(i, size, X[I[i]] = 0.0; );
   }
   // We only do one iteration here of things just so we can have things solved
   newton_solver->SetMaxIter(1);
   x = xprev;
   // We're getting our initial guess at change in velocity conditions
   // We don't care if our NR solver has converged or not. It doesn't matter.
   // Since once this function ends we, we go through the regular solver steps. 
   newton_solver->Mult(deltaF, x);
   // Reset our max number of iterations to our original desired value.
   newton_solver->SetMaxIter(newton_iter);
   // Tell the mech_operator to make use of the current Essential True Dofs
   mech_operator->UseEssTDofsCurrent();
}
