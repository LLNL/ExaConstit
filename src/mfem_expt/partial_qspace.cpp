...
const mfem::Vector& PartialQuadratureSpace::GetGeometricFactorWeights() const {
...
void PartialQuadratureSpace::ConstructMappings(std::shared_ptr<mfem::Mesh> mesh_,
...
PartialQuadratureSpace::PartialQuadratureSpace(std::shared_ptr<mfem::Mesh> mesh_,
                                               int order_,
                                               mfem::Array<bool>& partial_index)
...
PartialQuadratureSpace::PartialQuadratureSpace(std::shared_ptr<mfem::Mesh> mesh_, std::istream& in)
    ...
    in >> ident;
    MFEM_VERIFY(ident == "PartialQuadratureSpace", msg);
    in >> ident;
    MFEM_VERIFY(ident == "Type:", msg);
    in >> ident;
    if (ident == "default_quadrature") {
        in >> ident;
        MFEM_VERIFY(ident == "Order:", msg);
        in >> order;
    } else {
    ...
    in >> ident;
    MFEM_VERIFY(ident == "PartialIndices:", msg);
    int size;
    in >> size;
