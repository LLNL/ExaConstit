...
std::vector<std::array<double, 4>> GetSymmetryGroups(const LatticeType& lattice_type) 
{
    ...
    switch (lattice_type) {
        ...
        case LatticeType::RHOMBOHEDRAL: {
            ...
            lattice_symm = {
                {0.0, 1.0, 0.0, 0.0}, // identity
                {FRAC_PI_3 * 2.0, 1.0, 1.0, 1.0}, // 3-fold rotations about [111] direction (2 operations)
                {FRAC_PI_3 * 4.0, 1.0, 1.0, 1.0},
                {PI, 2.0 * ISQRT6, -ISQRT6, -ISQRT6}, // 2-fold rotations perpendicular to [111] (3 operations)
...
LightUp::LightUp(const std::vector<std::array<double, 3>> &hkls,
    ...
    m_lattice(lattice_params, lattice_type),
